...
func DrawCollisions(
	...
	for eA, cols := range collisions {
		...
		if !ecsContainer.Transforms.HasComponent(eA) {
...
func DrawColliders(
	...
	for _, e := range colManager.EntityIds(ecsContainer) {
		tm := ecsContainer.TransformManager
		...
		colShapes, err := colManager.GetShapes(e, ecsContainer)
		if err != nil {
...
func DrawAABBs(
	...
	for _, e := range colManager.EntityIds(ecsContainer) {
		...
		aabb, err := colManager.GetWorldPaddedAABB(e, ecsContainer)
		if err != nil {
